...
if __name__ == '__main__':
    ...
    cityies = ["miami","athens","buenosaires","stockholm","bengaluru","kampala"]
    for city in cityies:
        print('==========>', city)
        query_broken_files = []
        index_broken_files = []

        with open('/root/halo/code/MSFFT/datasets/MSLS_test/{}/{}_query_c.txt'.format(city, city)) as fid:
            query_lines = fid.read().splitlines()
            query_lines = [q.split(',')[0] for q in query_lines]

        with open('/root/halo/code/MSFFT/datasets/MSLS_test/{}/{}_db_c.txt'.format(city, city)) as fid:
            gallery_lines = fid.read().splitlines()
            gallery_lines = [q.split(',')[0] for q in gallery_lines]

        query_feats = []
        query_path = '/root/halo/code/MSFFT/datasets/MSLS_test/{}/delg_feats/query'.format(city)
        ...
        np.save('/root/halo/code/MSFFT/datasets/MSLS_test/{}/delg_feats/delg_qfeat'.format(city), query_feats)

        query_feats = query_feats / LA.norm(query_feats, axis=-1)[:,None]

        index_feats = []
        db_path = '/root/halo/code/MSFFT/datasets/MSLS_test/{}/delg_feats/index'.format(city)
        ...
        np.save('/root/halo/code/MSFFT/datasets/MSLS_test/{}/delg_feats/delg_dbfeat'.format(city), index_feats)
        ...
        np.save('/root/halo/code/MSFFT/datasets/MSLS_test/{}/delg_feats/delg_rank_index'.format(city), nn_inds)
