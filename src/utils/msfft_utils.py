...
def CalculateKeypointCenters(rf_boxes):
    ...
    '''
    xymin = torch.index_select(rf_boxes, dim=1, index=torch.LongTensor([0,1]).cuda(0))
    xymax = torch.index_select(rf_boxes, dim=1, index=torch.LongTensor([2,3]).cuda(0))
    return (xymax + xymin) / 2.0

def GetDtlgFeature(delg_features, delg_scores, scale, \
        receptive_field, stride, padding, attn_thres, topk):
    ...
    indices = None
    while(indices is None or indices.dim == 0 or len(indices) < topk):
        indices = torch.gt(delg_scores, attn_thres).nonzero().squeeze()
        attn_thres = attn_thres * 0.5   # use lower threshold if no indexes are found.
        if attn_thres < 0.01:
            break
    
    selected_boxes = torch.index_select(rf_boxes.cuda(0), dim=0, index=indices)
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    v, idx = scores.sort(0)  # 升序排序，idx代表下标
    # I = I[v >= 0.01]
